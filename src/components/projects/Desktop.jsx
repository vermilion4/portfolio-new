...
export default function Desktop() {
  ...
  return (
    ...
    {projects.map((project, index) => (
          <div
            key={index}
            className={`relative self-stretch flex flex-col w-[146px] h-[501px] max-md:max-w-full transition-all ease-in-out duration-500 ${
          ...
          >
            ...
            />
            {showProject === index && (
              <div className="absolute top-0 left-0 w-full h-full bg-black opacity-80 transition-all duration-500 grid place-content-center text-center text-white rounded-lg ease-in-out">
                {/* Add project details here */}
                <div className={`w-2/3 mx-auto ${index === projects.length - 1 && 'w-full'} `}>
                <p className="font-bold tracking-[0.48px] pb-2">
                  {project.title}
                </p>
                <p className="text-sm leading-6 tracking-[0.42px] ">
                  {project.description}
                </p>
                </div>
